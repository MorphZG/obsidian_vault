...
My dotfiles are copied to "~/.dotfiles" ($DOTFILES). To use Stow, each directory inside the .dotfiles must represent the home directory. Stow will follow the relative path when deploying a symlink. Each configuration directory inside the .dotfiles directory must hold the same relative path from the home directory. Reference: [[01_Reference/stow the dotfiles|stow the dotfiles]].
